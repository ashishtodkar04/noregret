...
class QuoteStore {
  ...
  static Future<void> init() async {
    ...
    // Load the saved index and date from disk
    final prefs = await SharedPreferences.getInstance();
    _persistedIndex = prefs.getInt('daily_quote_index');
    _cachedDateKey = prefs.getString('daily_quote_date');
  }
  ...
  static Quote getDailyQuote() {
    ...
    // If the date changed or we don't have a saved index, pick a new one
    if (_cachedDateKey != todayKey || _persistedIndex == null) {
      _persistedIndex = Random().nextInt(_quotes.length);
      _cachedDateKey = todayKey;
      _saveToDisk(_persistedIndex!, todayKey);
    }

    return _quotes[_persistedIndex! % _quotes.length];
  }

  static Future<void> _saveToDisk(int index, String dateKey) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('daily_quote_index', index);
    await prefs.setString('daily_quote_date', dateKey);
  }
}
